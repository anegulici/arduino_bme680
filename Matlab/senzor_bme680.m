...
arduino = serial('COM3','BaudRate',9600);    % Creates a serial connection through COM4
...
x = linspace(1,10,10);                              % x can be changed to alter the number of data points 
...
for i = 1:length(x)
    serial_read = fscanf(arduino,'%d');          % Reads to serial output as text
    serial_data(i) = fscanf(arduino,'%d');       % Saves the serial output as text
...
plot(t2, serial_data);              % Plots the serial_data from above vs the new time variable
